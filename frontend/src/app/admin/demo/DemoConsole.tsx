...
// 마일스톤 seq1~5 순차 집행(10/20/30/25/15%, 합계 4,400만, 잔여 0) + 스텝8 수수료 풀 회수금.
const STEPS: { step: number; title: string; detail: string }[] = [
  ...
  { step: 4, title: "마일스톤 1 · 계약 체결", detail: "공간사용 협약서 AI 판독 → 4,400,000원 집행 (10%)" },
  { step: 5, title: "마일스톤 2 · 설비 발주·제작", detail: "발주서·계약금 영수증 교차검증 → 8,800,000원 집행 (누적 30%)" },
  { step: 6, title: "마일스톤 3 · 반입·설치 착수", detail: "반입 현장 사진·운송 영수증 교차검증 → 13,200,000원 집행 (누적 60%)" },
  { step: 7, title: "마일스톤 4 · 설치 완료·검수", detail: "설치 완료 사진·검수확인서 교차검증 → 11,000,000원 집행 (누적 85%)" },
  { step: 8, title: "회수금 분배", detail: "월 매출 1,400,000원 → 수수료 풀 380,000원 · 투자자 회수금 228,000원(1좌 51원)" },
  { step: 9, title: "마일스톤 5 · 시운전·영업 개시", detail: "IoT 14일 가동률·첫 판매 영수증 검증 → 6,600,000원 집행 (잔여 0)" },
];
...
function headline(result: unknown): string {
  ...
  if ("verify" in r) {
    ...
    const complete = asObject(r.complete);
    return complete
      ? "AI 검증 통과 → 신탁 자금 집행 완료"
      : "AI 검증 통과 (집행 응답 없음)";
  }

  // /api/dividends/distribute 응답의 응답 — totalDividend(투자자 회수금 총액)·perToken(1좌당).
  const dividend = asObject(r.dividend);
  if (dividend) {
    const total = dividend.totalDividend;
    if (total == null) return "회수금 분배 완료";
    const perToken = dividend.perToken;
    return perToken != null
      ? `회수금 분배 완료 · 총 ${won(Number(total))} · 1좌당 ${won(Number(perToken))}`
      : `회수금 분배 완료 · 총 ${won(Number(total))}`;
  }
