...
const MONTHLY_PLATFORM_FEE_PER_SITE = 200_000; // v18 §2 ② 이용료 월 20만/사이트
const EXPERIENCE_BROKERAGE_RATE = 0.3; // v18 §2 ③ 체험 중개 30%
const B2B_SUCCESS_FEE_RATE = 0.08; // v18 §2 ④ B2B 신규계약 성사 8%
const INVESTOR_DIVIDEND_SHARE = 0.6; // v18 §2 수수료 풀의 60% 배당 / 40% FarmFi 운영
 ...
 *
 * @param _projectId 사이트 ID. 프로젝트 1개 = 사이트 1개라 이용료는 1개월분 고정.
 * @param operatorRevenue 해당 기간 운영자 작물 매출 (원). 체험 매출 추정에만 쓰인다.
...
export async function calculateFeePool(
  ...
  const platformUsageFee = MONTHLY_PLATFORM_FEE_PER_SITE;
  ...
  const experienceFee = Math.round(experienceRevenue * EXPERIENCE_BROKERAGE_RATE);
  ...
  const b2bFee = Math.round(b2bIncrementalRevenue * B2B_SUCCESS_FEE_RATE);

  const feePool = platformUsageFee + experienceFee + b2bFee;

  const investorDividend = Math.round(feePool * INVESTOR_DIVIDEND_SHARE);
