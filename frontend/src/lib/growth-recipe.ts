...
// 낮게 끌려간다.
//
// 초과시간을 회귀 열로 넣지 않는 이유는 자유도 하나다. 가중치로 쓰면 파라미터를
// 늘리지 않는다.
//
// "그 열을 넣으면 헤시안 대각이 0이 되어 판정이 항상 안장점"은 성립하지 않는다 —
// surfaceVerdict는 이차항 beta[7..12]와 상호작용 beta[13..15]만 읽으므로 17번째 열은
// 헤시안에 들어가지도 않는다. 실제로 넣어 돌리면 판정은 최대점이고 온도 회수도
// 가중보다 정확하다. 즉 정공법이 막혀 있는 게 아니라 자유도를 아끼려고 가중을 택한
// 것이고, 그 대가가 STRESS_HALF_HOURS에 대한 민감도다.
...
// 재표본 수는 판정을 결정할 만큼이어야 한다. 200회에서는 백분위 95% 구간이 4번째·
// 195번째 순서통계량이라 몬테카를로 오차가 크고, 같은 데이터에서 부트스트랩 시드만
// 바꿔도 n=30 기준 여섯 중 넷의 판정이 뒤집힌다(σ 변동 최대 68%).
//
// 값을 1000으로 둔 근거는 측정이다. 비용은 재표본당 16×16 가중 최소제곱 한 번이고
// n=200에서 314µs다 — 200회 95ms / 1000회 430ms / 2000회 857ms. 판정이 뒤집히는
// 요인 수는 200회 4/6 → 2000회 1/6으로 준다. 1000회는 그 사이의 지점이고, 리포트
// 한 번에 0.4초를 쓰는 대신 판정 안정성을 얻는다. 남은 몬테카를로 오차는 문서
// §10.2에 적어둔다 — 0으로 만든 게 아니라 값을 정하고 그 대가를 밝힌 것이다.
export const CURVATURE_REPLICATES = 1000;

export function curvatureUnresolved(
  Z: number[][],
  y: number[],
  w: number[],
  replicates = CURVATURE_REPLICATES,
  seed = 23
): boolean[] {
  const n = Z.length;
