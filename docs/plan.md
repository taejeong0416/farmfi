...
> **포지셔닝**: 본 프로젝트는 실제 증권을 발행하지 않는다(법적 비대상). STO 자금집행 메커니즘을 **작동하는 운영형 MVP**로 실증하는 것이 목표다.
> - **실배포**: 메커니즘 레이어(스마트컨트랙트 + AI 검증)는 Polygon Amoy 테스트넷에 실제 배포.
> - **mock 유지**: 결제 입출구(원화 입출금)·규제(KYC/AML·적격성·공시)는 명시적 모의 처리 — 법적으로 실제 구현 대상이 아니다.
> - **운영형 MVP 확장**: 단일 데모 PoC를 넘어 계정 인증·다중 프로젝트·온체인 진실화(인덱서)를 이번 범위로 승격한다 — `L1-11` 참고.
...
- [ ] `contracts/src/Escrow.sol`
  ...
  - `refund()` — **projectFailed 상태에서만** 남은 자금 투자자 비례 환불
    ...
    - 알려진 한계: 환불 후 토큰 미소각 (실서비스 전환 시 소각/블랙리스트 필요 → L1-12)
...
- 알려진 한계 (데모 범위에서 수용, 실서비스 시 L1-12에서 처리):
...
- [ ] `src/app/api/dividends/distribute/route.ts` — POST
  ...
  - 순서:
    ...
    4. **배당 자동 클레임 (데모 범위 결정)**: DividendClaim을 claimed=true로 생성 + 투자자 잔액 즉시 반영 + dividend 거래 기록 생성 — 라운드별 수동 클레임 UI는 실서비스 단계(L1-12)로
...
## L1-11. 운영형 MVP 확장 (PoC → 작동하는 플랫폼)

> 단일 데모 PoC를 "실제 운영처럼 동작하는 MVP"로 끌어올리는 항목. 원래 L1-12(해커톤 이후)에 있던 것 중 일부를 이번 범위로 **승격**했다.
> 단, 실결제·규제는 여전히 mock 유지 — "운영형"은 *동작 범위*를 넓힌다는 뜻이지 규제 대응이 아니다.

### L2-11-1. 계정·인증 (데모 드롭다운 → 실로그인)

> ⚠️ 이건 **계정 인증**이지 규제 KYC가 아니다. 로그인했다고 신원검증된 게 아니다(KYC는 L1-12 mock 유지).

#### L3 태스크
- [ ] 로그인 방식 결정 — **추천: 지갑 서명 로그인(SIWE, Sign-In with Ethereum)**. web3 흐름과 자연스럽고 비번/이메일 저장 불필요. (대안: 이메일+비밀번호)
- [ ] `User` 모델 확장: SIWE면 `nonce` 필드 / 이메일이면 `passwordHash` 필드 추가
- [ ] 세션 관리: NextAuth(SIWE provider) 또는 직접 JWT(httpOnly 쿠키)
- [ ] RBAC 가드: 투자자/운영자/건물주/관리자 역할별 API 접근 제어 — 미들웨어 또는 라우트별 `requireRole()`
- [ ] 관리자 페이지(`/admin`) 인증 보호 (현재 무인증 노출 → 관리자 role만 접근)
- [ ] 기존 데모 드롭다운은 dev/seed 계정 빠른 전환용으로 유지 (시연 편의)

### L2-11-2. 다중 프로젝트 (단일 데모 → 플랫폼)

> 목록/상세 API·UI는 이미 다중 대응(DB 쿼리). 시드만 1개라 **생성 플로우 + 컨트랙트 등록**만 추가하면 플랫폼이 된다.

#### L3 태스크
- [ ] 프로젝트 생성 API/폼 (운영자·관리자 role): name, location, 면적, 토큰수·가격, 마일스톤 4단계 정의
- [ ] 컨트랙트 연결 방식 결정 — **추천(MVP): 수동 등록 흐름** (관리자가 FarmToken/Escrow 배포 후 주소를 프로젝트에 등록). 대안 Factory 컨트랙트(자동 배포)는 가스·복잡도↑라 이후로.
- [ ] 프로젝트 라이프사이클 상태머신 정비: upcoming→funding→funded→operating→completed (전환 조건 코드화)
- [ ] 시연: seed 프로젝트 1건 + 신규 생성 1건으로 "여러 개가 동시에 굴러간다"를 보여줌

### L2-11-3. 온체인 진실화 + 이벤트 인덱서

> 현재: **DB가 진실 / 온체인은 증명 앵커**. 목표: **온체인이 진실 / DB는 인덱싱 캐시**. PoC↔플랫폼을 가르는 기술 척추.

#### L3 태스크
- [ ] `src/lib/indexer.ts` — viem `getLogs`로 핵심 이벤트 폴링 동기화
  - 구독 이벤트: `Subscribed`, `MilestoneVerified`, `TrancheReleased`, `DividendDistributed`
  - 마지막 처리 블록(`lastIndexedBlock`)부터 최신까지 조회 → DB 반영
- [ ] 인프라 결정 — **추천: Vercel Cron Jobs로 주기 폴링**(예: 1분). Vercel serverless는 상주 워커가 불가하므로 `watchEvent`(상주 웹소켓) 대신 cron `getLogs` 폴링이 적합.
  - [ ] `src/app/api/indexer/sync/route.ts` — cron이 호출하는 동기화 엔드포인트
  - [ ] `vercel.json`에 cron 등록
- [ ] 멱등성: 같은 이벤트 중복 반영 방지 — `(txHash, logIndex)` unique 또는 `lastIndexedBlock` 워터마크
- [ ] write 경로 재배치: 청약/트랜치/배당은 "온체인 tx 발생 → 인덱서가 DB 보정"을 진실원천으로. UI 즉시성은 낙관적 업데이트로 유지하되 인덱서가 최종 정합을 맞춤.
- [ ] 온체인 견고성(일부 승격): tx 재시도·nonce 관리·가스 추정 (서버 지갑)
- [ ] DB 모델: `IndexerState` (lastIndexedBlock) — 또는 이벤트 로그 테이블

---

## L1-12. 실서비스 확장 항목 (해커톤 이후 개발)

> ⚠️ **중요도가 낮아서가 아니라, 개발 시점을 뒤로 미루는 항목들이다.**
> 실제 STO 서비스로 가려면 대부분 필수다. 해커톤 데모 범위에서 빠졌을 뿐, 우선순위(중요도) 자체는 높다.
> 시점: 해커톤 종료 후 / 실서비스 전환 단계.

### L2-12-1. 인증·인가 — **→ L1-11로 승격(이번 범위)**
- (구현은 L1-11 참고)

### L2-12-2. 결제 입출구 (중요도: 높음, mock 유지)
- [ ] 원화 입금/출금 실연동 (현재 DB 잔액 시뮬레이션)
- [ ] PG사/가상계좌 연동
- [ ] 정산·환불 실제 송금 처리

### L2-12-3. 규제·컴플라이언스 (중요도: 높음, 법적 비대상 → mock 유지)
- [ ] KYC/AML (실명·신원 인증)
- [ ] 투자자 적격성 검증 (투자 한도)
- [ ] 공시·감사 로그 (거래·집행 내역 감사 추적)
- [ ] 약관·전자서명

### L2-12-4. 온체인 견고성 (중요도: 높음, 일부 L1-11 승격)
- [ ] 서버 지갑 키 관리 (KMS/HSM, 현재 .env PRIVATE_KEY)
- [ ] ~~트랜잭션 재시도·nonce 관리·가스 추정~~ → L1-11로 승격
- [ ] 메인넷 전환 (현재 Amoy 테스트넷)
- [ ] 컨트랙트 외부 감사(audit)

### L2-12-5. 운영·인프라 (중요도: 중간)
...
### L2-12-6. 확장 기능 (중요도: 중간)
- [ ] ~~다중 프로젝트 관리~~ → L1-11로 승격
- [ ] 토큰 2차 거래(유통시장)
