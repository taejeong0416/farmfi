...
### 다음 할 일
- **(🧑 사용자) faucet POL ~0.3 확보** (24h 제한) → 그 후 **(🤖) Amoy 배포 → `frontend/.env`에 `FARM_TOKEN_ADDRESS`/`ESCROW_ADDRESS` 기록 → 소액 부트스트랩 청약 1회 → 온체인 e2e 재검증**(txHash가 amoy.polygonscan에 뜨는지). 지갑 `0x535FD6…caF9`, 키는 `frontend/.env`·`contracts/.env`에 설정됨.
