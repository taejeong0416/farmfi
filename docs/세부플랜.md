...
- [ ] `prisma/schema.prisma` 작성 — 모델:
  ...
  - `Milestone` (id, projectId, seq, name, description, releasePct, releaseAmount, status, conditionText, evidenceUrl, aiVerificationResult, completedAt)
...
- [ ] `prisma/seed.ts` 작성:
  ...
  - 프로젝트 1개 시딩
    - "금정구 미니팜 1호", 심볼 MF01, 토큰가격 5000원, 총 토큰 1000개, 목표 500만원
    - status: 'funding', contractAddress: 배포된 FarmToken 주소
  - 에스크로 1개 시딩 (contractAddress: 배포된 Escrow 주소)
  - 마일스톤 3개 시딩
    - seq 1: "공사 완료", 40%, 200만원
    - seq 2: "첫 수확", 30%, 150만원
    - seq 3: "운영 안정", 30%, 150만원
...
- [ ] `src/lib/constants.ts`
  ...
  - `MilestoneStatus` enum: pending, in_progress, verified, completed, failed
...
- [ ] `contracts/test/Escrow.t.sol`
  ...
  - 전체 시나리오: subscribe → verify(1) → release(1) → verify(2) → release(2) → verify(3) → release(3) → remaining == 0
...
- [ ] `src/app/api/ai/verify-photo/route.ts` — POST
  - 요청: `{ milestoneId, imageBase64, milestoneType: 'construction' | 'harvest' }`
...
- [ ] `src/app/api/ai/detect-anomaly/route.ts` — POST
  ...
  - 응답: `{ anomalyDetected: boolean, anomalyScore: number, affectedSensors: [...] }`
...
- [ ] `src/app/api/milestones/[id]/verify/route.ts` — POST
  - 요청: `{ receiptImage?, photoImage?, milestoneType }`
  - 순서:
    1. 영수증 검증 API 호출 → passed 확인
    2. 사진 검증 API 호출 → passed 확인
    3. IoT 이상 탐지 API 호출 → anomalyDetected 확인
    4. 3개 신호 중 2개 이상 통과 시 최종 passed
  - 최종 passed → 온체인 `escrow.verifyMilestone(seq, true)` 트랜잭션 전송 (서버 지갑)
  - 최종 failed → `escrow.verifyMilestone(seq, false)` + 거부 사유 기록
  - DB에 milestone.aiVerificationResult 업데이트
  - 응답: `{ passed, receipt, photo, iot, txHash }`
...
- [ ] `src/app/api/subscribe/route.ts` — POST
  ...
  - **순서: 온체인 tx 성공 확인 후 DB 업데이트** (불일치 방지)
    1. 온체인 `subscribe()` 호출 → tx receipt 대기
    2. tx 실패 시 → 400 에러 반환 (DB 변경 없음)
    3. tx 성공 시 → Prisma 트랜잭션으로 DB 일괄 업데이트:
       - user.balance 확인 → 부족 시 400 에러
...
- [ ] `src/app/api/milestones/[id]/complete/route.ts` — POST
  ...
  - 트랜잭션 성공 시 DB 업데이트:
    ...
    5. 모든 마일스톤 completed → project.status = 'operating'

### L2-4-4. 배당 분배

#### L3 태스크
- [ ] `src/app/api/dividends/distribute/route.ts` — POST
  - 요청: `{ projectId, totalRevenue, dividendRatio }`
  - **온체인**: Dividend 컨트랙트의 `distributeDividend()` 호출
  - **오프체인**: DB에 배당 이력 기록 + 각 보유자별 claimAmount 계산
  - 응답: dividend 정보 + claims 배열 + txHash
...
- [ ] `src/app/admin/page.tsx`
  - 프로젝트 상태 요약 카드
  - **AI 검증 현황 섹션**
    - 각 마일스톤: AI 검증 상태 (미검증/통과/거부) + 검증 상세 결과
    - 검증 자료 업로드 (영수증 이미지, 공사·수확 사진)
  ...
  - **IoT 이상 알림 섹션**
    ...
    - 이상 감지 시 빨간 뱃지 표시
  - 배당 실행 섹션
    - 매출액 input + 배당비율 slider + 계산 미리보기 + [배당 실행] 버튼
  - 데모 리셋 섹션
...
- [ ] `src/components/dashboard/stat-cards.tsx`
  - 4개 카드: 에스크로 잔액 / 총 해제 금액 / 투자자 수 / 누적 배당
- [ ] `src/components/dashboard/fund-flow-chart.tsx`
  - Recharts BarChart
  - X축: 이벤트 (청약, 트랜치1~3), Y축: 금액
  - 입금 초록, 출금 주황
- [ ] `src/components/dashboard/milestone-progress.tsx`
  - 수평 3단계 스텝 인디케이터
  - 각 단계에 AI 검증 상태 아이콘 표시 (검증중 스피너 / 통과 체크 / 거부 X)
...
- [ ] `src/app/projects/[id]/dashboard/page.tsx`
  - 1행: StatCards
  - 2행: FundFlowChart (2/3) + MilestoneProgress (1/3)
  - 3행: IotCurrent (1/3) + IotChart (2/3)
  - 4행: GrowthStatus (1/3) + TxList (1/3) + EsgImpact (1/3)
...
- [ ] `src/app/api/demo/step/route.ts` — POST
  ...
  - step별 로직:
    ...
    - **4**: 마일스톤 1 AI 검증 (mock 영수증 + 사진) → 검증 통과 → 트랜치 해제 40%
    - **5**: 마일스톤 2 AI 검증 → 트랜치 해제 30%
    - **6**: 배당 분배 (매출 300만, 비율 70%)
    - **7**: 마일스톤 3 AI 검증 → 트랜치 해제 30% → operating
  - **실패 케이스 데모** (선택):
...
- [ ] `src/app/demo/page.tsx`
  - 좌측 패널 (1/3): 스텝 진행 목록 (7단계 + 실패 케이스 2개)
...
| ③ 마일스톤 통과 시 추가 승인 없이 자동 집행 | `POST /api/milestones/[id]/complete`: AI 통과 → 온체인 releaseTranche 자동 호출 | 데모 step 4~7: 검증 통과 → 자동 해제 → txHash 발급 |
