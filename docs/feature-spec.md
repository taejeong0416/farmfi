...
- 갭 분석: 실행 권고와 예상 수율 상방. "온도를 X℃ 올리면 수율 Y% 상방" 형태로 쓴다. 요인별 기여의 합이 전체 상방과 맞아야 한다.
- 수익 최적 레시피: 수율 최적점과 수익 최적점을 나란히. 요인별로 비용이 붙는지 표시한다.
- 근거 표시: 학습에 쓴 사이클 수와 관측 범위, 반응표면 판정, 설명력, 출처 배분(문헌·이전·자체). 표면이 최대점이 아니면 권고를 내지 않고 그 사유를 쓴다. 문헌이 지배하는 상태는 출처 배분에 그대로 드러나므로 별도 문구를 두지 않는다.
- 다음 실험 제안: 어떤 조건을 시험하면 추정이 가장 많이 개선되는지. 제안이 없으면 "추가 실험 없이 현 레시피를 써도 된다"를 쓴다 — 패널을 숨기지 않는다.
...
- 입력: 사이클별 환경 관측치(온도·습도·CO₂·EC·pH·DLI), 그 사이클의 수확량, **그 사이클의 품종**.
- 출력 다섯 가지.
  ...
  | 실험 제안 | 어떤 조건을 시험하면 추정이 가장 많이 개선되는지 |
...
- 산출은 반드시 **진단과 함께** 나간다. 반응표면이 최대점을 갖지 않으면(안장) 값을 내되 권고는 내지 않는다 — 그 값은 최적점이 아니라 탐색범위의 끝이다. 요인별로 최적점이 탐색범위 끝에 붙었는지도 표시한다. 표본이 모델 파라미터에 못 미치면 설명력은 0이 아니라 **판정 불가**다.
...
- 사후 불확실성이 문헌 사전의 일정 비율 이상 남은 요인만 실험으로 제안한다. 제안 지점은 **관측이 성긴 쪽**이다 — 이미 촘촘한 구간을 다시 재봐야 배우는 게 없다.
...
- 산출은 권고다. 적용 여부는 O-13에서 운영자가 정한다.
...
- `GET /spaces/{spaceId}/recipe` — 특성 중요도·최적 레시피(구간·출처 배분)·갭 분석·수익 최적 레시피·다음 실험 제안. 반응표면 판정과 설명력을 함께 실어, 받는 쪽이 권고를 쓸 수 있는지 판단할 수 있게 한다
