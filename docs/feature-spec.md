...
|---|---|
| STO, 토큰증권 | 프로젝트 투자 |
| 토큰 배정, 보유 토큰 | 투자 금액, 프로젝트 참여 내역, 보유 구좌 |
| 배당 | 회수금, 받은 수익, 지급 예정액 |
...
- 운영자: 공간 탐색, 신청, 현장 방문, 교육, 계약, 보증서, 개점 준비, **마일스톤 증빙 제출(O-11)**, **매장 운영 현황(O-12)**
- 관리자: 운영자 심사, 보증서 발급, 공간·설비 구성, 투자 프로젝트와 마일스톤 관리, **마일스톤 증빙 검토·승인(A-08)**, **매출·비용 입력(A-10)**
...
- 생육 레시피·환경 최적화 화면(O-13)과 기관 성과 리포트(A-11)
...
| O-12 매장 운영 현황 | 같은 데이터를 매장 밖 큰 화면에서 조회. **조작 기능 없음** |
| O-13 생육 레시피·환경 최적화 | 표·차트가 크고, 현장 조작이 아니라 정책을 바꾸는 화면 |
...
Next.js App Router 기준 경로다. 화면 정의는 4~8장에 있다.
...
| C-03 | 회원가입 | `/signup` |
| C-04 | 이용 목적 선택 | `/role` |
| C-I01 | 본인확인 방법 선택 | `/verify-identity` |
| C-I02 · C-I02E | 모바일 신분증 확인 · 실패 | `/verify-identity/mobile-id` |
| C-I03 | 본인 명의 계좌 확인 | `/verify-identity/account` |
| C-I05 | 투자 준비 완료 | `/verify-identity/done` |
| — | 프로젝트 목록 | `/projects` |
| I-01 | 프로젝트 상세 | `/projects/[projectId]` |
| I-02 · I-02E | 투자 적합성 확인 · 부적격 | `/projects/[projectId]/eligibility` |
| I-03 · I-03E | 최종 확인·전자서명·납입 · 실패 | `/projects/[projectId]/invest` |
| I-04 | 투자 신청 완료 | `/projects/[projectId]/invest/done` |
| I-06 | 투자자 홈 | `/mypage` |
| I-07 | 보유 투자·연결 계좌 | `/mypage/investments` |
| I-08 | 투자금 회수 상세 | `/mypage/payouts` |
| B-01 | 픽업 지점 선택 | `/subscribe` |
| B-02 | 팩 크기·수령 주기 | `/subscribe/plan` |
| B-03 | 지점 재고 기반 구성 | `/subscribe/items` |
| B-04 · B-04M | 주문서 확인 · 쿠폰 모달 | `/subscribe/order` |
| B-05 | 결제수단·자동결제 | `/subscribe/payment` |
| B-06 | 구독 신청 완료 | `/subscribe/done` |
| B-07 · B-00E | 내 구독 · 구독 없음 | `/mypage/subscription` |
| B-09 | 픽업 바코드 | `/mypage/subscription/pickup/[pickupId]` |
| O-01 | 운영 공간 탐색 | `/operator/spaces` |
| O-02 | 공간 상세·신청 시작 | `/operator/spaces/[spaceId]` |
| O-03 | 자격·서류 신청 | `/operator/apply` |
| O-04 | 현장 방문 예약 | `/operator/apply/visit` |
| O-05 | 필수 교육 | `/operator/education` |
| O-06 | 공간 최종 확정 | `/operator/apply/confirm` |
| O-07 | 계약 확인·전자서명 | `/operator/contract` |
| O-08 | 운영자 보증서 발급·앱 설치 | `/operator/credential` |
| O-09 | 개점 준비 현황 | `/operator/opening` |
| O-11 | 마일스톤 증빙 제출 (PDF·조회) | `/operator/milestones` |
| O-12 | 매장 운영 현황 | `/monitoring/[projectId]` |
| O-13 | 생육 레시피·환경 최적화 | `/optimization/[projectId]` |
| A-01 | 콘솔 홈 | `/admin` |
| A-02 | 운영자 심사·가배정 | `/admin/operators` |
| A-03 | 운영자 보증서 발급 | `/admin/credentials` |
| A-04 | 공간·설비 구성 | `/admin/spaces` |
| A-06 | 투자 프로젝트 관리 | `/admin/projects` |
| A-07 | 마일스톤 설정 | `/admin/projects/[projectId]/milestones` |
| A-08 | 마일스톤 증빙 검토·승인 | `/admin/milestones/review` |
| A-10 | 매출·비용 입력 | `/admin/projects/[projectId]/records` |
| A-11 | 기관 성과 리포트 | `/admin/reports/institution` |

`지갑 주소 등록`·`지갑 재연결` 경로는 만들지 않는다. 기존 URL로 들어오면 `/verify-identity/account` 또는 `/verify-identity/done`으로 리다이렉트한다.
...
flowchart TD
  ...
  O09 --> O12["O-12 매장 운영 현황 (조회)"]
  O12 --> O13["O-13 레시피·최적화"]
...
flowchart TD
  ...
  A01 --> A10["A-10 매출·비용 입력"]
  A01 --> A11["A-11 기관 성과 리포트"]
  A01 --> A08["A-08 증빙 검토·승인"]
  AUTO["9.7 자동 검증"] -.->|판정 초안| A08
...
| 역할 | 주요 권한 | 접근 제한 |
|---|---|---|
| 비회원 | 서비스 홈, 프로젝트 공개정보, 구독 플랜 조회 | 투자 신청·구독 결제·운영 신청 불가 |
...
- 예상 회수율은 프로젝트 API 값이 있을 때만 표시한다.
...
- 특성 중요도: 어느 환경 요인이 수율을 좌우하는지 순서대로.
- 최적 레시피: 6개 요인의 권장값과 현재값을 나란히. 차이가 큰 항목을 위로 올린다.
- 갭 분석: 실행 권고와 예상 수율 상방. "온도를 X℃ 올리면 수율 Y% 상방" 형태로 쓴다.
- 근거 표시: 학습에 쓴 사이클 수와 관측 범위. 3사이클 미만이면 "데이터 부족 — 문헌값 사용 중"으로 표시하고 권고를 내지 않는다.
- 다음 실험 제안: 어떤 조건을 시험하면 추정이 가장 많이 개선되는지.
- **적용 버튼**을 눌러야 목표값이 바뀐다. 자동 적용하지 않는다. 적용하면 9.2 최적대 판정과 아래 스케줄 탭이 새 목표를 쓴다.

**스케줄 탭 (9.5)**

- 현재 스케줄과 최적화가 제안한 스케줄을 나란히 보여준다.
- 입력: 목표 DLI(레시피 탭에서 적용된 값), 요금제(TOU) 구간, 계절.
...
- **자동 검증 초안:** 제출된 증빙에 대해 9.7의 자동 검증이 먼저 돌고, 그 결과가 조건 항목별 판정의 기본값으로 채워진다. 관리자는 항목마다 근거와 추출값을 보고 확인·수정한다. 자동 검증 실패는 반려가 아니라 `manual_review`다.
...
- 기관과 기간을 선택해 리포트를 만든다. 집계 항목은 9.9을 따른다.
...
| 데이터 | 수집 경로 | 주기 | 쓰이는 곳 |
|---|---|---|---|
| IoT 센서 | 설비 연동 · 게이트웨이 | 30분 | A-08 IoT 조건 판정, O-12 모니터링 |
| 수확 기록 | 운영자 앱 입력 (M-07) | 수확 시 | 생산량 집계, 기관 리포트 |
| 판매 기록 | 매장 판매·구독 픽업 | 건별 | I-08 정산 매출, 재고 차감 |
| 재고 | 판매·수확·수동 조정 | 변동 시 | O-12 부족 경보 |
| 비용 | A-10 관리자 입력 | 월 | I-08 정산 인정 비용 |
...
9.5가 "목표를 어떻게 싸게 달성하나"라면, 여기는 **"무엇을 목표로 할 것인가"**를 정한다. 두 절이 짝이다.

- 작물 프로파일의 최적대와 목표 DLI는 문헌값이라 **초기값일 뿐이다.** 매장마다 설비·구조·품종이 달라 실제 최적점이 다르다. 데이터가 쌓이면 학습값으로 대체한다.
- 입력: 사이클별 환경 관측치(온도·습도·CO₂·EC·pH·DLI)와 그 사이클의 수확량.
- 출력 세 가지.

  | 출력 | 내용 |
  |---|---|
  | 특성 중요도 | 어느 환경 요인이 수율을 좌우하는가 |
  | 최적 레시피 | 6개 요인의 결합 최적점. 요인 간 상호작용을 반영한다 |
...
- 데이터가 적은 초기에는 작물학 문헌값을 사전분포로 두고 관측이 쌓일수록 데이터 쪽으로 옮겨간다. 관측 3사이클 미만이면 학습값을 적용하지 않고 프로파일 값을 그대로 쓴다.
- 최적점 근처 데이터가 없으면 추정이 불확실하다. **다음에 어떤 조건을 시험해 보면 정보가 가장 많이 늘어나는지**를 함께 제안한다.
- 결과는 **권고이지 자동 적용이 아니다.** 운영자가 O-13에서 확인하고 적용을 눌러야 목표값이 바뀐다. 적용 이력을 남긴다.
- 적용된 목표값은 9.2의 최적대 판정과 9.5의 스케줄 산출에 그대로 쓰인다. 두 곳이 다른 목표를 보면 안 된다.
- 학습에 쓴 관측 범위와 사이클 수를 함께 표시한다. 운영자가 근거의 두께를 알 수 있어야 한다.

### 9.5 환경 최적화

- 목표 DLI를 유지하면서 전기요금이 싼 시간대로 광주기를 옮긴다. 시간을 압축하면 PPFD를 올려 적산을 맞춘다.
- 목표 DLI는 9.4에서 적용된 값을 쓴다. 학습값이 없으면 작물 프로파일 값을 쓴다.
- 계절·요금제(TOU) 입력을 받아 스케줄을 낸다.
- 과거 데이터로 백테스트하고 시나리오를 비교한다.
- 환경 조절(온도·CO₂·pH)은 제어 루프가 자동으로 한다. 사람이 개입하는 것은 예외 상황뿐이다.
- 최적화가 낸 스케줄과 실제 적용 결과를 함께 기록한다. 정산·판정에 쓰는 값은 실제 적용값이다.
...
- A-11에서 관리자가 발행하고, 기관은 전달받은 링크로 본다. 기관 계정 관리는 후속 범위다.
...
- `POST /milestones/{milestoneId}/verify` — 자동 검증 실행(9.7), 내부용
...
- `GET /spaces/{spaceId}/briefing` — 오늘 할 일(9.6)
- `GET /spaces/{spaceId}/recipe` — 특성 중요도·최적 레시피·갭 분석·다음 실험 제안
...
- `PUT /admin/projects/{projectId}/records` — 매출·비용 입력 (A-10)
...
11. 매출·비용 입력·확정(A-10) — I-08 정산 계산 입력
12. 마일스톤 자동 검증(9.7) — A-08 판정 초안
...
5. 운영자 서류 보완
6. 지급 실패와 계좌 수정
7. 매장 운영 현황(O-12) — 생육·환경·재고·매출 조회
...
2. 정산 규칙 편집
3. 권한 관리·알림 발송·AML
4. 생육 레시피·환경 최적화(O-13), 기관 성과 리포트(A-11), NAV 표시
...
- 사용자 화면 어디에도 STO, 토큰 배정, 지갑 주소, 고정 수익 보장 문구가 노출되지 않는다.
