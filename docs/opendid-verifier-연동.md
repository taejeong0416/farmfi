...
> ⚠️ **자격증명은 이 문서에 적지 않는다.** 오라클 서버의 8092·8099가 외부에
> 열려 있어(데모용) 관리자 비밀번호가 여기 있으면 그대로 서버가 열린다. 원문
> 기록에는 평문으로 있었고, main에 올리면서 전부 가렸다. 실제 값은 팀 비밀
> 저장소에서 받는다.
...
- **관리자 콘솔 로그인**(TAS/Issuer/Verifier 공통): `admin@opendid.omnione.net` / 비밀번호는 이 문서에 적지 않는다
  (8092·8099가 공개돼 있어 적는 순간 서버가 열린다. 값은 팀 비밀 저장소에서 받는다.)
...
- **admin 활성화**: 초기 admin은 미등록(비번=sha256("password"), require_password_reset=t). GUI가 비번을 **sha256 해시로 전송**하므로 DB `login_password`에 `sha256(<관리자 비밀번호>)` 저장 + reset 플래그 off.
- **지갑 연결 오류("Failed to connect to wallet")**: 각 서버 `jars/<E>/application.yml`의 `wallet:` 아래 `password: <지갑 비밀번호>` 누락. 추가+재시작으로 해결.
- **TA Registration secret**: TA 마법사 Step1 비번 = `ta.auth.registration-password`(번들 `application-auth.yml`에 들어 있다).
